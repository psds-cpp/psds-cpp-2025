...
private:
    int* data_;      // начало данных
    size_t size_;    // размер
    size_t capacity_;  // емкость != размер.
public:
    // Конструктор по умолчанию
    SimpleVector() : data_(nullptr), size_(0), capacity_(0) {}  // указатель на nullptr, размер = 0б емкость = 0ж

    // Конструктор, принимающий размер вектора и заполняющий его нулями
    SimpleVector(size_t size)

    // Конструктор, принимающий размер и заполняющий нулями
    explicit SimpleVector(size_t size) : data_(new size_t[size]), size_(size), capacity_(size) {
      size_t i == 0;  
      for(size_t i == 0, size < i, i++)  data_[i] = 0;
    }
